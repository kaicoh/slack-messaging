...
pub struct PlainTextError {
    ...
    pub text: Vec<ValidationErrorKind>,

    /// errors of emoji field
    pub emoji: Vec<ValidationErrorKind>,
}
...
mod tests {
    ...
    fn default_builder_returns_text_required_error() {
        ...
        let expected = PlainTextError {
            text: vec![ValidationErrorKind::Required],
            emoji: vec![],
        };
    ...
    fn builder_can_return_min_text_1_error() {
        ...
        let err = result.unwrap_err();
        let expected = PlainTextError {
            text: vec![ValidationErrorKind::MinTextLegth(1)],
            emoji: vec![],
        };
        assert_eq!(err, expected);
    }
    ...
    fn builder_can_return_max_text_3000_error() {
        ...
        let err = result.unwrap_err();
        let expected = PlainTextError {
            text: vec![ValidationErrorKind::MaxTextLegth(3000)],
            emoji: vec![],
        };
        assert_eq!(err, expected);
    }
